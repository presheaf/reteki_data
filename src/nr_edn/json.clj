  ...
  (let [
    ...
    (->> (for [[k s] sets]
           {:code (:code s)
            ...
            :name (:name s)
            :position (:position s)
            :size (if (= (:name s) "Equity and Eternity") (dec (:size s)) (:size s))})
         (sort-by :code)
         (map #(into (sorted-map) %))
         (#(json/generate-string % pretty))
         (#(str % "\n"))
    ...
    ;; pack/*.json
    (let* [skipped-cards #{"Consolidation"} ; we are cheating to have this as a separate reteki card, but we don't want it to exist on NRDB
           packs
           (->> (for [card (filter #(not (skipped-cards (:title %))) cards)
